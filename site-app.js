...
(function () {
  ...
  /* ---- the sticky tab bar, and the tab that just became current ----------- */
  (function stickyTabs() {
    var wrap = document.querySelector(".tabwrap");
    if (!wrap) return;
    // A zero-height sentinel above the bar: when it leaves the viewport the bar
    // has pinned. Cheaper and steadier than reading scroll position every frame.
    var mark = document.createElement("div");
    mark.setAttribute("aria-hidden", "true");
    mark.style.cssText = "position:absolute;height:1px;width:1px;pointer-events:none";
    ...
    // Pulse whichever tab has just become current, in both bars.
    var seen = {};
    new MutationObserver(function (recs) {
      recs.forEach(function (r) {
        var el = r.target;
        if (!el.classList || !el.classList.contains("tab")) return;
        var on = el.getAttribute("aria-current") === "true";
        if (on && !seen[el.id || el.textContent]) {
          el.classList.remove("just-set");
          void el.offsetWidth;                 /* restart the animation */
          el.classList.add("just-set");
          setTimeout(function () { el.classList.remove("just-set"); }, 700);
        }
        seen[el.id || el.textContent] = on;
      });
